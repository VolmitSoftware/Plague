...
public class PlagueCommandFunction
{
	...
	public String invoke(PlagueCommand instance, PlagueSender sender, KList<String> s)
	{
		...
		for(int i = 0; i < parameters.size(); i++)
		{
			...
			if(last)
			{
				if(par.isVararg())
				{
					...
					fitted.add(objects.toArray(new Object[objects.size()]));
					continue;
				}
			}
		...
		{
			fitted.add(0, sender);
			method.invoke(instance, fitted.toArray(new Object[fitted.size()]));
		}
	...
	public Object force(Class<?> type, String object) throws Throwable
	{
		...
		{
			return ((Integer) force(Integer.class, object)).intValue();
		}
		...
		{
			return ((Float) force(Float.class, object)).floatValue();
		}
		...
		{
			return ((Double) force(Double.class, object)).doubleValue();
		}
		...
		{
			return ((Long) force(Long.class, object)).longValue();
		}
	...
	public boolean isArgumentCountSupported(int count)
	{
		...
		if(maxParameters >= 0 && count > maxParameters)
		{
			return false;
		}
